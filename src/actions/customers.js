...
export const startAddCustomer = (customerData = {}) => {
    return (dispatch, getState) => {
        ...
        database.ref(`users/${uid}/customers`).push(customer).then((ref) => {
...
export const startRemoveCustomer = ({ id } = {}) => {
    return (dispatch, getState) => {
        const uid = getState().auth.uid;
        database.ref(`users/${uid}/customers/${id}`).remove().then(() => {
...
export const startEditCustomer = (id, state) => {
    return (dispatch, getState) => {
        ...
        database.ref(`users/${uid}/customers/${id}`).set(state).then(() => {
        })
    };
};

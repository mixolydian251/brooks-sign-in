...
class ViewCustomerPage extends React.Component {
    ...
    showVisits = (number) => {
        ...
        const generateVisitItems = (i) => {
            if (moment(visits[i]).format('YYYY-M-D') === moment().format('YYYY-M-D')){
                return <p className="today" key={visits[i]}>{moment(visits[i]).format('MMM Do, YYYY')}</p>
            }
    ...
    signIn = () => {
        if (this.props.customer.visits === false){
            this.props.dispatch(startEditSingleProp(this.props.customer.id, {
                visits: [Number(moment().format('x'))]
            }))
        }
        else if (moment(this.props.customer.visits[0]).format('YYYY-MM-DD') !==
    ...
    addShoes = () => {
        ...
        } else { this.props.dispatch(startEditSingleProp(this.props.customer.id, {
        ...
        return (
            <div className="view_customer_wrapper">
                <div className="view_customer_items">
                    <div className="item"><h1>{this.props.customer.name}</h1></div>
                    <div className="item"><p><b>Age:</b>{` ${this.props.customer.age}`}</p></div>
                    <div className="item">
                        <p><b>Address:</b>
                    ...
                    <div className="item"><p><b>Phone Number:</b> { this.props.customer.phoneNumber }</p></div>

                    <div className="item">
                        <b><p>Dependants:</p></b>

                        {this.props.customer.dependants !== false && this.props.customer.dependants.map((dependant) => {
                            return <p key={dependant.name}>{`${dependant.name}, ${dependant.age}`}</p>
                        })}
                    ...
                    <div className="item">
                        <p><b>Number of visits this year:</b>
                            <span>{this.props.customer.visits !== false ? this.props.customer.visits.filter( visit =>
                                moment(visit).format('YYYY') === moment().format('YYYY') ).length : 0 }</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="limited_item_layout">
                    <div className="limited_item">
                        <h3><b>Coats this year:</b>
                            <span>{this.props.customer.coats !== false ? this.props.customer.coats.filter( createdAt =>
                                moment(createdAt).format('YYYY') === moment().format('YYYY') ).length : 0 }</span>
                        </h3>
                        <button className="coat_button" onClick={this.addCoat}>Add Coat</button>
                    </div>


                    <div className="limited_item">
                        <h3><b>Shoes this year:</b>
                            <span>{this.props.customer.shoes !== false ? this.props.customer.shoes.filter( createdAt =>
                                moment(createdAt).format('YYYY') === moment().format('YYYY') ).length : 0}</span>
                        </h3>
                        <button className="shoes_button" onClick={this.addShoes}>Add Pair of Shoes</button>
                    </div>
                ...
                <div className="button_layout">
                    <button className="sign_in_button" onClick={this.signIn}>Sign in customer</button>
